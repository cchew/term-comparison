...
- **v0.1.2** - Trust/reliability pass: deterministic fallback headline when the LLM difference-summary is unavailable (non-flagship terms no longer silently show nothing), persistent "not an official government service" disclaimer, new `?`-triggered "How this works" modal (pipeline, accuracy approach, known limitations, scope), Escape-to-close + focus handling on the modal, e2e/a11y coverage for both.
